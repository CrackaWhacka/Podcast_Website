...
@pytest.fixture
def my_csv_podcast(my_csv_reader):
    
    # Pulling one instance of the podcast object from the CSVData
    return my_csv_reader.podcast_list[1]

def test_create_author(my_csv_reader):
    author = Author(CSVDataReader.author_id, "name")
    assert isinstance(author, Author)
    assert author.name == "name"
    assert repr(author) == "<Author 983: name>" #Its 982 diff authors in the CSV file but +1 for "name".

def test_create_catagory(my_csv_reader):
    cat = Category(CSVDataReader.category_id, "name")
    assert isinstance(cat, Category)
    assert cat.name == "name"
    assert repr(cat) == "<Category 2167: name>" #Its 2166 diff categories in the CSv file but +1 for "name".
...
def test_csv_reader_podcast(my_csv_reader):
    ...
    assert repr(my_csv_reader.podcast_list[1]) == "<Podcast 726: '101.3 Sea FM - Gawndy & Ash Pollard Catch Up' by hit101.3 Sea FM Central Coast>"
    
def test_csv_reader_podcast_attributes(my_csv_reader, my_csv_podcast):
    
    # Testing for the attributes from the Podcast object that was listed in the memory_repo
    assert my_csv_podcast.id == 901
    assert repr(my_csv_podcast.author) == "<Author 1865: Radio France>"
    assert repr(type(my_csv_podcast.author)) == "<class 'podcast.domainmodel.model.Author'>"
    assert my_csv_podcast.title == "100% Sports France Bleu Champagne-Ardenne"
    assert my_csv_podcast.image == "http://is3.mzstatic.com/image/thumb/Music128/v4/d0/14/ee/d014eecf-2a5c-577d-cffe-329753311eec/source/600x600bb.jpg"
    assert my_csv_podcast.description == "100% Sports France Bleu Champagne-Ardenne"
    assert my_csv_podcast.language == "French"
    assert repr(my_csv_podcast.categories[0]) == "<Category 4095: Sports & Recreation>"
    assert repr(type(my_csv_podcast.categories[0])) == "<class 'podcast.domainmodel.model.Category'>"
    assert my_csv_podcast.episodes == []
    
    # Testing for podcasts that do have episodes
    podcast_2 = my_csv_reader.podcast_list[2]
    assert repr(type(podcast_2.episodes[0])) == "<class 'podcast.domainmodel.model.Episode'>"
    assert podcast_2.episodes[0].episode_name == "BREE + GAWNDY PODCAST - MONDAY 04.12.2017"
